	...
	createHeroInfoTemplate(hero) {
		const heroInfoTemplate = `<div class="hero" id="hero">
          ...
          <div data-slot="Secondary weapon" class="slot">
						<img src="${hero.subWeapon.image}" alt="shield">	
					</div>
          ...
          <div data-slot="Range" class="slot statistic">
			...
			hero.getSubRange ? '/ ' + hero.getSubRange() : ''
		} </div>
      </div>
      <div class="character">
...
class ArenaTemplate extends TemplateRoot {
	...
	setFighters(arena, old) {
		...
		document.getElementById(`pos${arena.hero.x}${arena.hero.y}`).innerHTML =
			this.makeHeroImage(arena);

		arena.monsters.forEach((monster, index) => {
			document.getElementById(`pos${monster.x}${monster.y}`).innerHTML =
				this.makeMonsterImage(arena, index);
		});
	}
